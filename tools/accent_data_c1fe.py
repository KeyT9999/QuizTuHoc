# -*- coding: utf-8 -*-

C1FE_ACCENTED = [
    {
        "qNum": 1,
        "text": "Nguồn gốc xã hội dẫn đến sự ra đời của Triết học là gì?",
        "options": {
            "A": "Do sự phát triển của lực lượng sản xuất",
            "B": "Cách mạng nổ ra, những tộc người tan rã",
            "C": "Lao động trí óc đã tách khỏi lao động chân tay",
            "D": "Phân tích đúng tình hình trên toàn thế giới, sản xuất phát triển"
        },
        "ans": "C"
    },
    {
        "qNum": 2,
        "text": "Triết học trong xã hội có giai cấp được hiểu như thế nào?",
        "options": {
            "A": "Cũng có tính giai cấp",
            "B": "Không có tính giai cấp",
            "C": "Chỉ triết học phương Tây mới có tính giai cấp",
            "D": "Chỉ có triết học phương Đông mới có tính giai cấp"
        },
        "ans": "A"
    },
    {
        "qNum": 3,
        "text": "Theo triết học Mác-xít thì siêu hình và biện chứng là hai phương pháp như thế nào?",
        "options": {
            "A": "Đối lập nhau",
            "B": "Tương tác lẫn nhau",
            "C": "Bổ sung cho nhau",
            "D": "Có mối liên hệ với nhau"
        },
        "ans": "A"
    },
    {
        "qNum": 4,
        "text": "Một trong những nguồn gốc ra đời của Triết học là:",
        "options": {
            "A": "Tự nhiên",
            "B": "Lao động",
            "C": "Ngôn ngữ",
            "D": "Xã hội"
        },
        "ans": "D"
    },
    {
        "qNum": 5,
        "text": "Trước khi chủ nghĩa duy vật biện chứng ra đời, nhìn chung các nhà triết học duy vật quan niệm vật chất là gì?",
        "options": {
            "A": "Một hay một số chất tự có, đầu tiên",
            "B": "Toàn bộ vật chất",
            "C": "Vũ trụ quan",
            "D": "Giới tự nhiên"
        },
        "ans": "A"
    },
    {
        "qNum": 6,
        "text": "Ai được xem là nhà triết học thiên tài vào thế kỷ XVIII - XIX?",
        "options": {
            "A": "Heraclitus",
            "B": "Immanuel Kant",
            "C": "Socrates",
            "D": "Bruno Fesnades"
        },
        "ans": "B"
    },
    {
        "qNum": 7,
        "text": "Đâu là khẳng định đúng về mối quan hệ giữa các bộ phận cấu thành chủ nghĩa Mác - Lênin?",
        "options": {
            "A": "Triết học là cơ sở nền tảng của kinh tế chính trị và chủ nghĩa xã hội khoa học.",
            "B": "Kinh tế chính trị học là cơ sở nền tảng của triết học và chủ nghĩa xã hội khoa học.",
            "C": "Chủ nghĩa xã hội khoa học là cơ sở của kinh tế chính trị và triết học.",
            "D": "Triết học, kinh tế chính trị và chủ nghĩa xã hội khoa học là những bộ phận độc lập, tách rời nhau."
        },
        "ans": "A"
    },
    {
        "qNum": 8,
        "text": "Triết học Mác - Lênin là thế giới quan khoa học của giai cấp nào?",
        "options": {
            "A": "Giai cấp tư sản tiến bộ",
            "B": "Giai cấp công nhân",
            "C": "Tầng lớp trí thức",
            "D": "Giai cấp lao động"
        },
        "ans": "B"
    },
    {
        "qNum": 9,
        "text": "Thế nào là phương pháp siêu hình?",
        "options": {
            "A": "Xem xét các sự vật trong mối liên hệ tác động qua lại lẫn nhau, nhưng không vận động, không phát triển",
            "B": "Xem xét các sự vật trong trạng thái cô lập, tách rời nhau, và không vận động, không phát triển",
            "C": "Xem xét các sự vật trong mối liên hệ tác động qua lại lẫn nhau, và luôn vận động, luôn phát triển",
            "D": "Xem xét các sự vật trong trạng thái cô lập, tách rời nhau, nhưng luôn vận động, luôn phát triển"
        },
        "ans": "B"
    },
    {
        "qNum": 10,
        "text": "Triết học Mác - Lênin là gì?",
        "options": {
            "A": "Triết học Mác - Lênin là hệ thống quan điểm duy vật biện chứng về tự nhiên, xã hội và tư duy - thế giới quan tiến bộ trong nhận thức và cải tạo thế giới",
            "B": "Khoa học làm cầu nối giữa các khoa học",
            "C": "Chính trị",
            "D": "Một thứ tôn giáo"
        },
        "ans": "A"
    },
    {
        "qNum": 11,
        "text": "Cái bàn, cái ghế có phải là vật chất với tư cách là một phạm trù triết học hay không?",
        "options": {
            "A": "Có. Vì nó là dạng của vật chất trong không gian",
            "B": "Không. Vì nó không có vật chất trong không gian.",
            "C": "Có. Vì nó có tồn tại cụ thể, có kích thước và tồn tại cùng thời gian.",
            "D": "Không. Vì nó chỉ là một dạng cụ thể của vật chất."
        },
        "ans": "D"
    },
    {
        "qNum": 12,
        "text": "Sự sinh ra, lớn lên, già đi thuộc hình thức vận động nào trong triết học?",
        "options": {
            "A": "Vận động cơ học",
            "B": "Vận động sinh học",
            "C": "Vận động vật lý",
            "D": "Vận động hóa học"
        },
        "ans": "B"
    },
    {
        "qNum": 13,
        "text": "Thời gian phát hiện ra tia Rơnghen là khi nào?",
        "options": {
            "A": "Năm 1895",
            "B": "Năm 1896",
            "C": "Năm 1897",
            "D": "Năm 1898"
        },
        "ans": "A"
    },
    {
        "qNum": 14,
        "text": "Theo cách phân chia các hình thức vận động của Ph.Ăngghen, hình thức vận động nào là thấp nhất?",
        "options": {
            "A": "Hình thức vận động cơ học",
            "B": "Hình thức vận động vật lý",
            "C": "Hình thức vận động hóa học",
            "D": "Hình thức vận động xã hội"
        },
        "ans": "A"
    },
    {
        "qNum": 15,
        "text": "Phép biện chứng là phương pháp nghiên cứu, khái quát thế giới thành hệ thống các nguyên lý, quy luật là phép biện chứng gì?",
        "options": {
            "A": "Biện chứng chủ quan",
            "B": "Biện chứng khách quan",
            "C": "Siêu hình học",
            "D": "Ý muốn con người"
        },
        "ans": "A"
    },
    {
        "qNum": 16,
        "text": "Theo quan điểm chủ nghĩa duy vật biện chứng thì đứng im là một trạng thái đặc biệt của vận động gì?",
        "options": {
            "A": "Tuyệt đối",
            "B": "Trong cân bằng",
            "C": "Tại chỗ",
            "D": "Tĩnh tại"
        },
        "ans": "B"
    },
    {
        "qNum": 17,
        "text": "Luận điểm \"Mối liên hệ nhân quả là do cảm giác con người quy định\" thuộc lập trường triết học nào?",
        "options": {
            "A": "Chủ nghĩa duy tâm khách quan",
            "B": "Chủ nghĩa duy tâm chủ quan",
            "C": "Chủ nghĩa duy vật biện chứng",
            "D": "Chủ nghĩa duy vật siêu hình"
        },
        "ans": "B"
    },
    {
        "qNum": 18,
        "text": "Quan điểm toàn diện đặt ra yêu cầu phải xem xét tất cả các mối liên hệ của sự vật để làm gì?",
        "options": {
            "A": "Chống quan điểm siêu hình",
            "B": "Chống quan điểm duy tâm",
            "C": "Chống quan điểm chủ nghĩa chiết trung và thuyết ngụy biện",
            "D": "Đề phòng cho chúng ta khỏi phạm sai lầm và sự cứng nhắc"
        },
        "ans": "D"
    },
    {
        "qNum": 19,
        "text": "Ném một đồng xu có hai mặt đen và trắng lên trời, đồng xu rơi xuống đất và ngửa mặt đen lên trên. Hiện tượng trên gọi là:",
        "options": {
            "A": "Ngẫu nhiên",
            "B": "Tất nhiên",
            "C": "Vừa tất nhiên vừa ngẫu nhiên",
            "D": "Không phải tất nhiên và cũng không phải ngẫu nhiên"
        },
        "ans": "A"
    },
    {
        "qNum": 20,
        "text": "Nguồn gốc tự nhiên của ý thức bao gồm yếu tố nào?",
        "options": {
            "A": "Thế giới vật chất của con người",
            "B": "Thế giới khách quan tác động lên bộ não người",
            "C": "Bộ não người",
            "D": "Lao động và ngôn ngữ"
        },
        "ans": "B"
    },
    {
        "qNum": 21,
        "text": "Theo quan điểm của phép biện chứng duy vật thì những thay đổi về chất của sự vật lại tạo ra những thay đổi gì của sự vật trên các phương diện khác nhau?",
        "options": {
            "A": "Biến đổi mới về lượng",
            "B": "Biến đổi của chất",
            "C": "Chất mới",
            "D": "Cái mới"
        },
        "ans": "A"
    },
    {
        "qNum": 22,
        "text": "Nguyên nhân là một phạm trù triết học chỉ cái gì?",
        "options": {
            "A": "Tác động qua lại giữa các mặt, các yếu tố bên trong sự vật hay giữa sự vật này với sự vật kia tạo ra những biến đổi nhất định",
            "B": "Quan hệ giữa các mặt, các yếu tố bên trong sự vật hay giữa sự vật này với sự vật kia tạo ra những biến đổi nhất định",
            "C": "Sự chuyển hóa giữa các mặt, các yếu tố bên trong sự vật hay giữa sự vật này với sự vật kia tạo ra những biến đổi nhất định",
            "D": "Chỉ những cái có trước và luôn luôn quyết định cái sau"
        },
        "ans": "A"
    },
    {
        "qNum": 23,
        "text": "Cơ sở chủ yếu nhất để hình thành nên khái niệm là gì?",
        "options": {
            "A": "Thế giới tự nhiên",
            "B": "Thực tiễn",
            "C": "Nhận thức",
            "D": "Lý luận"
        },
        "ans": "B"
    },
    {
        "qNum": 24,
        "text": "Điều khẳng định nào sau đây đúng về mối quan hệ giữa lý luận và kinh nghiệm?",
        "options": {
            "A": "Lý luận được hình thành từ kinh nghiệm trên cơ sở kinh nghiệm",
            "B": "Kinh nghiệm nhiều tự phát dẫn đến sự ra đời của lý luận",
            "C": "Mọi lý luận đều được xuất phát từ kinh nghiệm",
            "D": "Lý luận đi trước kinh nghiệm"
        },
        "ans": "A"
    },
    {
        "qNum": 25,
        "text": "Theo quan điểm của triết học Mác-Lênin, tiêu chuẩn của chân lý là gì?",
        "options": {
            "A": "Tri thức do các thế hệ trước để lại",
            "B": "Tri thức được nhiều người công nhận",
            "C": "Thực tiễn",
            "D": "Lời nói của các vĩ nhân"
        },
        "ans": "C"
    },
    {
        "qNum": 26,
        "text": "Theo C. Mác, vấn đề tìm hiểu xem tư duy của con người có thể đạt được chân lý khách quan hay không hoàn toàn không phải là vấn đề lý luận mà là vấn đề gì?",
        "options": {
            "A": "Sống còn",
            "B": "Hiện thực",
            "C": "Thực tiễn",
            "D": "Khoa học"
        },
        "ans": "C"
    },
    {
        "qNum": 27,
        "text": "Trong hoạt động nhận thức, cần nhìn nhận sự vật trong trạng thái vận động và phát triển. Điều đó dựa trên cơ sở lý luận của nguyên lý nào?",
        "options": {
            "A": "Nguyên lý về mối liên hệ phổ biến",
            "B": "Nguyên lý về sự phát triển",
            "C": "Nguyên lý về tính thống nhất vật chất của thế giới",
            "D": "Nguyên lý thống nhất và đấu tranh giữa các mặt đối lập"
        },
        "ans": "B"
    },
    {
        "qNum": 28,
        "text": "Cơ sở, động lực, mục đích của nhận thức là gì?",
        "options": {
            "A": "Lý luận",
            "B": "Thực tiễn",
            "C": "Triết học",
            "D": "Tinh thần"
        },
        "ans": "B"
    },
    {
        "qNum": 29,
        "text": "Theo chủ nghĩa duy vật lịch sử thì những biến đổi từ đâu tạo ra nhu cầu khách quan phải có sự biến đổi tương ứng trong kiến trúc thượng tầng?",
        "options": {
            "A": "Cơ sở hạ tầng",
            "B": "Lực lượng sản xuất",
            "C": "Quan hệ sản xuất",
            "D": "Phương thức sản xuất"
        },
        "ans": "A"
    },
    {
        "qNum": 30,
        "text": "Trong tư liệu lao động, bộ phận nào cần được phát triển đi trước một bước so với đầu tư sản xuất trực tiếp?",
        "options": {
            "A": "Kết cấu hạ tầng sản xuất",
            "B": "Công cụ sản xuất",
            "C": "Các bộ phận chứa đựng, bảo quản",
            "D": "Các phương án trả lời đều đúng"
        },
        "ans": "A"
    },
    {
        "qNum": 31,
        "text": "Theo quan điểm duy vật lịch sử thì xã hội có các loại hình sản xuất cơ bản nào?",
        "options": {
            "A": "Sản xuất ra của cải vật chất, tinh thần và văn hóa",
            "B": "Sản xuất ra văn hóa, con người và đời sống tinh thần",
            "C": "Sản xuất ra của cải vật chất, đời sống tinh thần và nghệ thuật",
            "D": "Sản xuất ra của cải vật chất, tinh thần và con người"
        },
        "ans": "D"
    },
    {
        "qNum": 32,
        "text": "Theo quan điểm duy vật lịch sử, sản xuất ra của cải vật chất giữ vai trò là:",
        "options": {
            "A": "Nền tảng của xã hội",
            "B": "Nền tảng vật chất của xã hội",
            "C": "Nền tảng tinh thần của xã hội",
            "D": "Nền tảng kỹ thuật, công nghệ của xã hội"
        },
        "ans": "A"
    },
    {
        "qNum": 33,
        "text": "Theo V.I.Lênin, để có quan niệm đúng, vững chắc về sự phát triển của các hình thái kinh tế - xã hội là một quá trình lịch sử - tự nhiên thì cần phải:",
        "options": {
            "A": "Quy những mối quan hệ xã hội vào những quan hệ sản xuất",
            "B": "Quy những quan hệ sản xuất vào các quan hệ chính trị, pháp luật",
            "C": "Quy những quan hệ xã hội vào những quan hệ sản xuất và quy những quan hệ sản xuất vào trình độ phát triển của kỹ thuật, công nghệ hiện thời",
            "D": "Quy những quan hệ xã hội vào những quan hệ sản xuất và quy những quan hệ sản xuất vào trình độ phát triển của lực lượng sản xuất"
        },
        "ans": "D"
    },
    {
        "qNum": 34,
        "text": "Theo triết học Mác, sự chuyển dịch từ hình thái kinh tế chiếm hữu nô lệ, sang phong kiến rồi đến xã hội chủ nghĩa thuộc loại vận động nào?",
        "options": {
            "A": "Vận động vật lý",
            "B": "Vận động hóa học",
            "C": "Vận động sinh học",
            "D": "Vận động xã hội"
        },
        "ans": "D"
    },
    {
        "qNum": 35,
        "text": "Sự ra đời của quan điểm duy vật về lịch sử do C. Mác sáng lập ra đã khắc phục được những sai lầm căn bản trong việc lý giải xã hội theo:",
        "options": {
            "A": "Quan điểm tôn giáo và duy tâm",
            "B": "Quan điểm duy vật siêu hình và tôn giáo",
            "C": "Quan điểm duy tâm và siêu hình",
            "D": "Quan điểm duy vật tự nhiên và tầm thường"
        },
        "ans": "C"
    },
    {
        "qNum": 36,
        "text": "Theo chủ nghĩa duy vật biện chứng, giữa LLSX và QHSX có mối quan hệ như thế nào với nhau?",
        "options": {
            "A": "Biện chứng",
            "B": "Một chiều",
            "C": "Độc lập",
            "D": "Tách rời"
        },
        "ans": "A"
    },
    {
        "qNum": 37,
        "text": "Đâu là một trong những hình thức cộng đồng người trước khi hình thành dân tộc?",
        "options": {
            "A": "Thị tộc",
            "B": "Chủng tộc",
            "C": "Sắc tộc",
            "D": "Hệ tộc"
        },
        "ans": "A"
    },
    {
        "qNum": 38,
        "text": "Đấu tranh giai cấp có vai trò gì đối với sự phát triển của xã hội có đối kháng giai cấp?",
        "options": {
            "A": "Kìm hãm sự phát triển của xã hội",
            "B": "Duy trì sự tồn tại xã hội hiện tồn",
            "C": "Ảnh hưởng tiêu cực đến xã hội",
            "D": "Là động lực thúc đẩy"
        },
        "ans": "D"
    },
    {
        "qNum": 39,
        "text": "Quan hệ giữa người với người trong quá trình sản xuất được gọi là gì?",
        "options": {
            "A": "Quan hệ xã hội",
            "B": "Quan hệ tổ chức quản lý",
            "C": "Quan hệ sản xuất",
            "D": "Quan hệ điều tiết"
        },
        "ans": "C"
    },
    {
        "qNum": 40,
        "text": "Ngày nay, trong nền sản xuất xã hội, tỷ trọng lao động trí tuệ có xu hướng như thế nào?",
        "options": {
            "A": "Tăng dần",
            "B": "Giảm dần",
            "C": "Điều hòa",
            "D": "Ổn định"
        },
        "ans": "A"
    },
    {
        "qNum": 41,
        "text": "Bản chất của nhà nước là gì?",
        "options": {
            "A": "Cơ quan phúc lợi chung của toàn xã hội",
            "B": "Cơ quan trọng tài thực hiện chức năng phân xử và hòa giải các xung đột xã hội",
            "C": "Công cụ của giai cấp thống trị xã hội",
            "D": "Công cụ của giai cấp bị trị"
        },
        "ans": "C"
    },
    {
        "qNum": 42,
        "text": "Theo quan điểm duy vật lịch sử, nguồn gốc sâu xa của các cuộc cách mạng là do đâu?",
        "options": {
            "A": "Quần chúng lao động bị áp bức",
            "B": "Quần chúng lao động hiểu được lý luận chính trị",
            "C": "Giai cấp cầm quyền bị khủng hoảng về đường lối cai trị",
            "D": "Mâu thuẫn gay gắt giữa lực lượng sản xuất và quan hệ sản xuất"
        },
        "ans": "D"
    },
    {
        "qNum": 43,
        "text": "Đâu là một phát minh vĩ đại của C. Mác đã đem lại một cuộc cách mạng trong triết học về xã hội?",
        "options": {
            "A": "Chủ nghĩa duy vật lịch sử",
            "B": "Chủ nghĩa duy tâm khách quan",
            "C": "Chủ nghĩa duy tâm chủ quan",
            "D": "Chủ nghĩa duy vật chất phác"
        },
        "ans": "A"
    },
    {
        "qNum": 44,
        "text": "Nhà nước mang bản chất của giai cấp nào sau đây?",
        "options": {
            "A": "Mang bản chất của giai cấp thống trị",
            "B": "Mang bản chất của giai cấp bị trị",
            "C": "Mang bản chất của tất cả các giai cấp",
            "D": "Mang bản chất của xã hội"
        },
        "ans": "A"
    },
    {
        "qNum": 45,
        "text": "Chủ nghĩa duy vật lịch sử cho rằng sự ra đời, tồn tại của nhà nước là để duy trì trật tự xã hội trong điều kiện mâu thuẫn giai cấp như thế nào?",
        "options": {
            "A": "Đi đến giải quyết",
            "B": "Phải được giải quyết",
            "C": "Có thể giải quyết",
            "D": "Không thể điều hòa được"
        },
        "ans": "D"
    },
    {
        "qNum": 46,
        "text": "Hoạt động sản xuất vật chất là cơ sở hình thành nên quan hệ gì?",
        "options": {
            "A": "Quan hệ kinh tế giữa người với người",
            "B": "Quan hệ vật chất giữa người với người",
            "C": "Quan hệ kinh tế - vật chất giữa người với vật",
            "D": "Quan hệ kinh tế - vật chất giữa người với người"
        },
        "ans": "D"
    },
    {
        "qNum": 47,
        "text": "Tính chất kế thừa của ý thức xã hội làm cho nó có tính chất nào sau đây?",
        "options": {
            "A": "Tính phản ánh hư ảo",
            "B": "Tính phê phán",
            "C": "Tính độc lập tương đối",
            "D": "Tính hiện thực"
        },
        "ans": "C"
    },
    {
        "qNum": 48,
        "text": "Theo quan điểm của triết học Mác - Lênin, \"tồn tại xã hội\" bao gồm những yếu tố nào?",
        "options": {
            "A": "Dân số, ý thức xã hội, kiến trúc thượng tầng",
            "B": "Lực lượng sản xuất, dân số, phương thức sản xuất, ý thức xã hội",
            "C": "Môi trường tự nhiên, quan hệ sản xuất, tâm lý xã hội, hệ tư tưởng",
            "D": "Môi trường tự nhiên, dân số, phương thức sản xuất"
        },
        "ans": "D"
    },
    {
        "qNum": 49,
        "text": "Hạt nhân cơ bản trong quần chúng nhân dân là ai?",
        "options": {
            "A": "Những người lao động sản xuất ra của cải vật chất",
            "B": "Những bộ phận dân cư chống lại giai cấp thống trị áp bức bóc lột, đối kháng với nhân dân",
            "C": "Những tầng lớp xã hội khác nhau thúc đẩy sự tiến bộ của xã hội",
            "D": "Những tầng lớp dân cư lao động"
        },
        "ans": "A"
    },
    {
        "qNum": 50,
        "text": "Theo quan điểm của chủ nghĩa duy vật lịch sử, lực lượng chủ thể sáng tạo ra lịch sử là ai?",
        "options": {
            "A": "Quần chúng nhân dân",
            "B": "Vĩ nhân, lãnh tụ",
            "C": "Tầng lớp trí thức",
            "D": "Trí thức, vĩ nhân"
        },
        "ans": "A"
    },
    {
        "qNum": 51,
        "text": "Từ định nghĩa con người của Ph.Ăngghen, đặc điểm nào sau đây cho thấy con người khác với động vật?",
        "options": {
            "A": "Biết tư duy",
            "B": "Biết ứng xử theo các quy phạm đạo đức",
            "C": "Chính trị",
            "D": "Biết chế tạo và sử dụng công cụ lao động"
        },
        "ans": "D"
    },
    {
        "qNum": 52,
        "text": "Theo quan điểm chủ nghĩa duy vật lịch sử, lực lượng cơ bản nhất trong quần chúng nhân dân là gì?",
        "options": {
            "A": "Giai cấp thống trị xã hội",
            "B": "Tầng lớp trí thức",
            "C": "Người lao động",
            "D": "Công nhân và nông dân"
        },
        "ans": "C"
    },
    {
        "qNum": 53,
        "text": "Luận điểm sau đây thuộc lập trường triết học nào: \"Cái mới ra đời trên cơ sở phá hủy hoàn toàn cái cũ\"?",
        "options": {
            "A": "Quan điểm siêu hình",
            "B": "Quan điểm duy tâm",
            "C": "Quan điểm phát triển",
            "D": "Quan điểm biện chứng duy vật"
        },
        "ans": "A"
    },
    {
        "qNum": 54,
        "text": "Một sinh viên chuẩn bị bài seminar trong 1 tuần theo cách thức: ngày đầu phác thảo đề cương sơ lược sau đó hàng ngày dành 15 phút sửa chữa và bổ sung bài chuẩn bị đó cả kết cấu, nội dung. Đúng vào buổi seminar, sinh viên đó đã có một bản chuẩn bị hoàn chỉnh. Công việc trên đã diễn ra theo nguyên lý, quy luật nào?",
        "options": {
            "A": "Quy luật mâu thuẫn",
            "B": "Quy luật lượng - chất",
            "C": "Quy luật phủ định của phủ định",
            "D": "Nguyên lý về mối liên hệ phổ biến"
        },
        "ans": "B"
    },
    {
        "qNum": 55,
        "text": "Công ty sản xuất thuốc bảo vệ thực vật muốn bổ nhiệm anh Nguyễn Văn A và chị Trần Thị B làm trưởng phòng quản lý nhân sự. Trong quá trình xem xét, công ty đó đã căn cứ vào năng lực tích lũy trong quá trình làm việc, độ nhạy bén khi xử lý công việc, năng lực phán đoán của Anh A và Chị B... Căn cứ vào ví dụ trên, Anh (Chị) hãy cho biết công ty đó đã vận dụng nội dung nào sau đây để bổ nhiệm nhân sự?",
        "options": {
            "A": "Nguyên lý về mối liên hệ phổ biến",
            "B": "Quy luật từ những sự thay đổi về lượng dẫn tới sự thay đổi về chất và ngược lại.",
            "C": "Quy luật phủ định của phủ định",
            "D": "Quy luật đấu tranh và thống nhất giữa các mặt đối lập"
        },
        "ans": "B"
    },
    {
        "qNum": 56,
        "text": "Tại sao khi nhìn lên trời, mọi người đều nhìn thấy đám mây với những hình thù khác nhau?",
        "options": {
            "A": "Sự phản ánh năng động, sáng tạo của ý thức",
            "B": "Do ý muốn chủ quan của mỗi người",
            "C": "Thế giới khách quan đa dạng phong phú",
            "D": "Do đám mây hôm đó có hình thù như vật cụ thể nên mọi người nhìn khác nhau"
        },
        "ans": "A"
    },
    {
        "qNum": 57,
        "text": "Khi đánh giá một con người, quan điểm toàn diện đòi hỏi điều gì?",
        "options": {
            "A": "Phải đặc biệt nhấn mạnh một mặt nào đó, bỏ qua những mặt còn lại",
            "B": "Phải đặt họ vào những điều kiện, thời đại của mình mà đánh giá",
            "C": "Phải đặt họ trong những mối quan hệ với những người khác, với những việc khác có thể ảnh hưởng đến sự phát triển của họ mà đánh giá",
            "D": "Phải xuất phát từ mục đích và lợi ích của họ mà đánh giá"
        },
        "ans": "C"
    },
    {
        "qNum": 58,
        "text": "Cái bàn cái ghế là vật chất. Đây là quan niệm của các nhà triết học thời kỳ nào?",
        "options": {
            "A": "Duy vật chất phác thời cổ đại",
            "B": "Duy tâm chất phác thời cổ đại",
            "C": "Duy vật trung đại",
            "D": "Duy tâm trung đại"
        },
        "ans": "A"
    },
    {
        "qNum": 59,
        "text": "Thế giới quan duy vật biện chứng bao gồm:",
        "options": {
            "A": "Tri thức khoa học, niềm tin khoa học và lý tưởng cách mạng",
            "B": "Tri thức khoa học, niềm tin khoa học và hệ giá trị cộng đồng",
            "C": "Tri thức khoa học, tình cảm khoa học và lý tưởng khoa học",
            "D": "Tri thức khoa học, niềm tin khoa học và lý tưởng khoa học"
        },
        "ans": "A"
    },
    {
        "qNum": 60,
        "text": "Đâu được coi là quy luật cơ bản nhất quy định sự vận động và phát triển của xã hội?",
        "options": {
            "A": "Quy luật quan hệ sản xuất phù hợp với trình độ phát triển của lực lượng sản xuất",
            "B": "Biện chứng giữa cơ sở hạ tầng và kiến trúc thượng tầng",
            "C": "Sự phát triển của các hình thái kinh tế - xã hội là một quá trình lịch sử tự nhiên",
            "D": "Tất cả phương án trên"
        },
        "ans": "A"
    }
]
